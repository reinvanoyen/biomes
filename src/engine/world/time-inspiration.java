...
public class GameTimeClock extends Actor {
   ...
   public GameTimeClock(Skin skin, Assets assets, Timer timer, Label clockText) {
      this.worldTime = timer;
      this.sprite = new Sprite(assets.getTextureById(ImageID.IID_CLOCK));
      this.sprite.setPosition(this.getX(), this.getY());
      this.setBounds(0, 0, 128, 64);
   ...
   public void act(float delta) {
      ...
      if (lerp != null) {
         ambient.lerp(lerp, (float) amt);
                ...
                // update the time label if set
      if (text != null) {
         text.setText("Day " + (worldTime.getDaysPast() + 1) + "   "
               + worldTime.getFormattedTimeofDay());
      }
   }
...
// drawn method omitted
...
